...
//creo nuevas variables en este caso query3 y realizo la consulta a mi base de datos:
$consultas3 = "SELECT * FROM librosprado WHERE id_Libro = '$id'";
$query3 = mysqli_query($conexion, $consultas3);
...
<body>
  <div class="container pt-4" style="background-color: #92817a; font-family: 'Castoro', serif; height:100px;">
  <!--Con el método POST me redirijo a mi archivo procesar-eliminar donde se elige un libro para borrar:-->
    <form class="table table-dark" action="procesar_eliminar.php" method="POST">
      <?php
      while ($filas = $lista = mysqli_fetch_array($query3)) { ?>
      ...
      <?php } ?>
      <input name="eliminar" type="submit" value="Eliminar" class="btn btn-warning ml-2">
    </form>
  </div>
